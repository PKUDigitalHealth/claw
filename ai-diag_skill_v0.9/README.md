...
Data arrays contain **raw ADC integer values**. The API converts them to millivolts using:

```
voltage_mV = (adcValue - adcZero) / adcGain
```

`adcGain` = ADC units per 1 mV; `adcZero` = ADC value at 0 mV. These must match your recording device.

### Single-lead JSON

| Field | Type | Required | Description |
|---|---|---|---|
| `ecgData` | number[] | Yes | Raw ADC sample array |
| `ecgSampleRate` | number | Yes | Sampling rate in Hz (e.g. 500) |
| `adcGain` | number | Yes | ADC units per 1 mV |
| `adcZero` | number | Yes | ADC value at 0 mV baseline |

### 12-lead JSON

| Field | Type | Required | Description |
|---|---|---|---|
| `dataI` … `dataV6` | number[] | Yes | 12 lead raw ADC sample arrays |
| `ecgSampleRate` | number | Yes | Sampling rate in Hz |
| `adcGain` | number | Yes | ADC units per 1 mV (shared for all leads) |
| `adcZero` | number | Yes | ADC value at 0 mV baseline |
...
- ECG data is sensitive medical data — handle with care
