...
Analyze ECG data via the heartvoice (心之声) cloud API. Automatically selects the correct endpoint (1-lead or 12-lead) based on user intent, reads data from a user-specified JSON file, and returns structured diagnostic results in the user's language.
...
Detect intent from the user message and select the endpoint:

| User Intent | CN Keywords | EN Keywords | Mode Flag |
|---|---|---|---|
| Single-lead signal | 单导联、单导、1导联 | 1-lead, single lead, one lead | `1-lead` |
| 12-lead signal | 十二导联、12导联、多导联 | 12-lead, twelve lead | `12-lead` |

**If the user does not specify a type**, ask:
- Chinese: "请问您要分析的是单导联信号还是十二导联信号？"
- English: "Would you like to analyze a single-lead or 12-lead signal?"

**Language rule**: Detect the user's language from their message and respond entirely in that language throughout the session.

## Workflow

1. Identify analysis type (1-lead / 12-lead) from the user message.
2. Extract file path from the user message.
3. Run the appropriate script command with the correct `--lang` flag.
4. Parse the structured JSON output.
5. Present the result as a natural-language report in the user's language.
...
The data arrays contain **raw ADC integer values**, not physical voltage. The API internally converts them to millivolts (mV) using:

```
voltage_mV = (adcValue - adcZero) / adcGain
```

- `adcZero` — the ADC value that corresponds to 0 mV (baseline offset)
- `adcGain` — the number of ADC units per 1 mV

**Example**: if `adcGain = 1000`, `adcZero = 0`, and a sample value is `512`, then `voltage = (512 - 0) / 1000 = 0.512 mV`.

These two parameters must match your recording device's ADC configuration; incorrect values will produce wrong voltage readings and unreliable diagnoses.

### 1-lead JSON

```json
{
  "ecgData": [512, 515, 520, 518, 525, 530, 528, 535],
  "ecgSampleRate": 500,
  "adcGain": 1.0,
  "adcZero": 0.0
}
...
|---|---|---|---|
| `ecgData` | number[] | Yes | Raw ADC sample array (integer values, converted to mV via the formula above) |
| `ecgSampleRate` | number | Yes | Sampling rate in Hz (e.g. 500) |
| `adcGain` | number | Yes | ADC units per 1 mV — used to convert raw samples to voltage |
| `adcZero` | number | Yes | ADC value corresponding to 0 mV baseline |
...
| Field | Type | Required | Description |
|---|---|---|---|
| `dataI` … `dataV6` | number[] | Yes | 12 lead raw ADC sample arrays (I, II, III, aVR, aVL, aVF, V1–V6), converted to mV via the formula above |
| `ecgSampleRate` | number | Yes | Sampling rate in Hz |
| `adcGain` | number | Yes | ADC units per 1 mV — shared across all 12 leads |
| `adcZero` | number | Yes | ADC value corresponding to 0 mV baseline |
...
- ECG data is **sensitive medical data** — do not log or store raw signal data unnecessarily
